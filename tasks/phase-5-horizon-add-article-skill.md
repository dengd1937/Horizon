...
在 Horizon 仓库内实现项目级 `horizon-add-article` skill：在 Codex Desktop 中接收一篇或多篇文章 URL，调用外部 `baoyu-fetch` 抓取正文，生成结构保真的简体中文译文，再由 Horizon 确定性代码校验内容。用户可以选择在仓库外生成隔离的本地站点预览，或生成 `articles/{slug}.md`，经审阅后精确 commit & push，触发 Phase 2 已有文章发布 CI。skill 源码随 Horizon 维护，不建立独立 repo；个人环境中的安装方式不属于本阶段交付。
...
- [x] 实现结构化入库接口：接收一篇或多篇 source URL 的抓取正文和元数据，规范化 `source_domain`，生成 `added_date` 与 slug，安全序列化 YAML，并原子写入 `articles/{slug}.md`。
...
- [x] 接收单个或多个 URL 及可选 tags / intro，定位 Codex Desktop 当前 Horizon workspace，并用 Git 根目录、`pyproject.toml` 项目名和文章契约文件验证仓库身份；workspace 不匹配或无写权限时给出明确提示并停止。
...
- [x] 将用户批准绑定到单篇文章或整批文章的文件哈希、完整 diff、提交信息、当前 HEAD、目标 remote/branch；提交前重新校验，任一值变化都必须重新展示 diff 并再次确认。
- [x] 获得批准后只暂存并提交经审阅的文章路径；单篇提交信息精确使用 `clip(article): {slug}`，批量提交精确使用 `clip(articles): import {count} articles`，禁止 `git add .`，最终 commit patch 必须与用户批准的 diff 完全一致。
...
- [x] 单元测试批量入库和批量审阅：任一输入无效、URL 重复或 slug 冲突时不留下半成品；一次批准的 batch commit 仅包含审阅过的文章路径。
...
- [x] 真实 CI 验收只使用经用户授权的 Horizon `main` 和一篇确实待发布的文章；push 后查询 Actions run，断言 `workflow=publish-articles.yml`、`event=push`、`head_sha` 等于本次 commit、`head_branch=main`，并记录 run URL 与最终 conclusion。workflow 已限制为 `main`，测试分支不会触发生产部署，不能替代本项验收。

Codex Desktop 已在全新 Horizon 任务中确认两个 skill 均已注册：`baoyu-url-to-markdown` 从 `~/.agents/skills/baoyu-url-to-markdown/SKILL.md` 加载，`horizon-add-article` 经用户级软连接解析到当前 Horizon 工作副本中的权威 `skills/horizon-add-article/SKILL.md`。live smoke test 使用 Anthropic `Claude takes research to new places`、知乎 `python OR-Tools解决约束规划问题`、微信公众号 `如何用《玉树芝兰》入门数据科学？` 三个公开页面；临时抓取正文分别为 6,084、4,440、14,319 字，均通过共享契约 `load_article`、详情页与索引页渲染，未写入正式 `articles/`。本地端到端验收再次使用 Anthropic 页面，在 `/private/tmp` 中完成抓取、简体中文翻译、结构保真校验和站点生成，通过 `127.0.0.1` 打开详情页；浏览器确认中文标题、章节、段落、列表、链接文字和图片说明均正常，原始链接与媒体未丢失。桌面端实测正文栏、普通段落、一个视频和两张正文图片宽度均为 680px，视频按约 16:9 加载，所有媒体均未超出正文栏。

真实 CI 验收使用经用户审阅批准的 Anthropic `Building Effective AI Agents`：文章 commit 为 `4d63e849072eec459ba73a45d164fa389a44ae32`，只包含 `articles/anthropic-com-20241219-building-effective-agents.md`。按该 SHA 查询到 [`publish-articles.yml` run 29428429540](https://github.com/dengd1937/Horizon/actions/runs/29428429540)，其 `event=push`、`head_branch=main`、`head_sha=4d63e849072eec459ba73a45d164fa389a44ae32`、最终 `conclusion=success`；job 从 COS 恢复 118 个既有对象，生成 2 个文章页面、本地化 9 个媒体文件并成功完成部署。验收还发现并修复了 `create` CLI 缺少抓取校验器导入、以及多 remote 工作区中 workflow 查询误选 `upstream` 的问题；两项修复均加入回归测试，全量测试为 429 项通过。
